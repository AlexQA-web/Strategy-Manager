...
class QuikPy:
    ...
    def __init__(self, host='127.0.0.1', requests_port=34130, callbacks_port=34131):
        ...
        _cb_thread = Thread(target=self.callback_handler, name='CallbackThread', daemon=True)  # daemon=True — поток завершается при краше/выходе из программы
        _cb_thread.start()
        self.callback_thread = _cb_thread
    ...
    def process_request(self, request):
        ...
        :returns: Ответ JSON
        """
        self.lock.acquire()  # Ждем, пока не освободится блокировка
