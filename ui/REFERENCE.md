...
При отображении PnL, equity и комиссий в UI — **не вычислять** формулы самостоятельно.
Использовать `ValuationService` из `core/valuation_service.py`:
...
Для данных о позициях и ордерах — только read-only доступ через `order_history.get_order_pairs()`,
`order_history.get_total_pnl()`. Запись — исключительно через `FillLedger` из `core/`.
